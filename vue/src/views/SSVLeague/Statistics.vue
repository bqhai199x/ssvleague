            ...
            <tbody class="tw-bg-white">
              <tr v-for="(item, index) in topGoalScore" :key="item" class="hover:tw-opacity-70 hover:tw-bg-slate-50 tw-cursor-pointer tw-border">
                ...
                <td class="tw-flex tw-py-2 tw-whitespace-nowrap tw-items-center">
                  <img class="tw-w-10 tw-rounded-md" :src="item.avatar" :alt="item.player">
      ...
      <div class="tw-shadow tw-overflow-hidden sm:tw-rounded-lg">
        <table class="tw-min-w-full tw-text-sm tw-overflow-auto">
          <thead class="tw-text-sm">
            ...
            <tbody class="tw-bg-white">
              <tr v-for="(item, index) in topAttackClub" :key="item" class="hover:tw-opacity-70 hover:tw-bg-slate-50 tw-cursor-pointer tw-border">
                <td class="tw-pl-4 tw-w-[50px]">
                  <div class="tw-underline tw-text-blue-500 tw-font-bold">{{ index + 1 }}</div>
                </td>
                <td class="tw-flex tw-py-2 tw-whitespace-nowrap tw-items-center">
      ...
      <div class="tw-shadow tw-overflow-hidden sm:tw-rounded-lg">
        <table class="tw-min-w-full tw-text-sm tw-overflow-auto">
          <thead class="tw-text-sm">
            ...
            <tbody class="tw-bg-white">
              <tr v-for="(item, index) in topConcededClub" :key="item" class="hover:tw-opacity-70 hover:tw-bg-slate-50 tw-cursor-pointer tw-border">
                <td class="tw-pl-4 tw-w-[50px]">
                  <div class="tw-underline tw-text-blue-500 tw-font-bold">{{ index + 1 }}</div>
                </td>
                <td class="tw-flex tw-py-2 tw-whitespace-nowrap tw-items-center">
...
const topConcededClub = ref([]);
const topAttackClub = ref([]);
...
onMounted(async() => {
  ...
  [ 
    ...
    topBadPlay.value,
    favoriteClub.value,
    topConcededClub.value,
    topAttackClub.value
  ] = await Promise.all([
    ssvLeagueCaller.getTopGoalScore(),
    ssvLeagueCaller.getTopConceded(),
    ssvLeagueCaller.getTopBadPlay(),
    ssvLeagueCaller.getFavoriteClub(),
    ssvLeagueCaller.getTopConcededClub(),
    ssvLeagueCaller.getTopAttackClub()
  ]);
  loading.hideMulti();
})
</script>